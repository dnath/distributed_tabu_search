...
class Message:
  ...
  def deserialize(json_string):
    ...
    if not json_message_obj.has_key('message_type') or not isinstance(json_message_obj['message_type'], MessageTypes):
      raise MessageError('Invalid Message bytes ! Cannot deserialize json_string = {0}'.format(json_string))
    
    json_message_obj = MessageTypes(json_message_obj['message_type'])
